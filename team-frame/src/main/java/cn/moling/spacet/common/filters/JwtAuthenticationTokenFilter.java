...
import cn.moling.spacet.common.security.SecurityUserDetailsServiceImpl;
import cn.moling.spacet.utils.*;
...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        String authToken = "";
        //先从cookie中去取token，取不到再从header中取token
        String cookieToken = CookieUtil.getValue(request,"token");
//        cookieToken = null;
        String tokenHeader = request.getHeader(JwtTokenUtil.TOKEN_HEADER);
        if(cookieToken != null){
            authToken = cookieToken;
        }else if (tokenHeader != null && tokenHeader.startsWith(JwtTokenUtil.TOKEN_PREFIX)) {
            authToken = tokenHeader.substring(JwtTokenUtil.TOKEN_PREFIX.length());
        }
        ...
        if (!"".equals(authToken)) {
            ...
            if (redisUtil.isBlackList(authToken)) {
                ...
                response.getWriter().write(JSON.toJSONString(new CommandResult(ResultEnum.TOKEN_IS_BLACKLIST, false)));
                return;
            }
            ...
            if (redisUtil.hasKey(authToken)) {//判断redis是否有保存
                ...
                if (JwtTokenUtil.isExpiration(expirationTime)) {
                    ...
                    if (DateUtil.compareDate(currentTime, tokenValidTime)) {
                        ...
                        response.getWriter().write(JSON.toJSONString(new CommandResult(ResultEnum.LOGIN_IS_OVERDUE, false)));
                        return;
                        ...
                        response.setHeader(JwtTokenUtil.TOKEN_HEADER, JwtTokenUtil.TOKEN_PREFIX + jwtToken);
                    }
                }
